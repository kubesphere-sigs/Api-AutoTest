...
class TestCluster(object):
    ...
    def test_get_cluster_version(self):
        for cluster_name in self.cluster_names:
            ...
            # 验证获取版本号成功
            assert cluster_version == 'v3.2.0-alpha.1.19+edcd8e1449a14c'
    ...
    def test_get_cluster_roles(self):
        for cluster_name in self.cluster_names:
            ...
            r = multi_cluster_steps.step_get_cluster_roles(cluster_name)
            # 验证查询成功
    ...
    def test_query_non_existent_pod(self):
        for cluster_name in self.cluster_names:
            ...
            re = multi_cluster_steps.step_query_pod(cluster_name, node_name, pod_name)
            # 获取查询结果
    ...
    def test_precise_query_existent_pod(self):
        for cluster_name in self.cluster_names:
            ...
            node_name = res.json()['items'][0]['metadata']['name']
            # 获取节点的任意一个pod的名称
            re = multi_cluster_steps.step_get_pod_of_node(cluster_name, node_name)
            pod_name = re.json()['items'][0]['metadata']['name']
            # 按名称精确查询存在的pod
            r = multi_cluster_steps.step_query_pod(cluster_name, node_name, pod_name)
            # 获取查询到的pod名称
    ...
    def test_fuzzy_query_existent_pod(self):
        for cluster_name in self.cluster_names:
            ...
            r = multi_cluster_steps.step_query_pod(cluster_name, node_name, pod_fuzzy_name)
            # 获取查询到的pod名称
            name_actual = r.json()['items'][0]['metadata']['name']
            # 验证查询结果
            assert name_actual == pod_name
    ...
    def test_precise_non_existent_system_project(self):
        for cluster_name in self.cluster_names:
            ...
            # 查询指定的集群的系统项目
            re = multi_cluster_steps.step_query_system_project(cluster_name, project_name)
            # 获取查询结果
            items = re.json()['items']
            # 验证查询结果为空
            assert items == []
    ...
    def test_precise_query_existent_system_project(self):
        for cluster_name in self.cluster_names:
            # 获取集群任意系统项目的名称
            re = multi_cluster_steps.step_query_system_project(cluster_name, '')
            project_name = re.json()['items'][0]['metadata']['name']
            # 按名称精确查询系统项目
            r = multi_cluster_steps.step_query_system_project(cluster_name, project_name)
            # 获取查询结果
    ...
    def test_fuzzy_query_existent_system_project(self):
        for cluster_name in self.cluster_names:
            ...
            r = multi_cluster_steps.step_query_system_project(cluster_name, fuzzy_project_name)
            # 获取查询结果
            project_name_actual = r.json()['items'][0]['metadata']['name']
            # 验证查询结果
            assert project_name_actual == project_name
    ...
    def test_get_one_system_project_quota(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, project_count):
                ...
                project_name = re.json()['items'][j]['metadata']['name']
                # 查询项目的配额信息
                r = multi_cluster_steps.step_get_project_quota(cluster_name, project_name)
                # 获取项目的配额信息
    ...
    def test_get_one_system_project_detail(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, project_count):
                ...
                project_name = re.json()['items'][j]['metadata']['name']
                # 查询项目的LimitRanges
                r = multi_cluster_steps.step_get_project_limit_ranges(cluster_name, project_name)
                # 获取请求资源的kind
                kind = r.json()['kind']
    ...
    def test_get_system_project_workload(self):
        for cluster_name in self.cluster_names:
            # 获取集群中系统项目的数量
            res = multi_cluster_steps.step_query_system_project(cluster_name, '')
            ...
            for j in range(0, project_count):
                ...
                project_name = re.json()['items'][j]['metadata']['name']
                # 查询项目的工作负载信息
                r = multi_cluster_steps.step_get_project_workload(cluster_name, project_name)
                # 获取接口的响应数据
                data = r.json()['data']
                # 验证接口响应数据正确
    ...
    def test_check_system_project_pods(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, project_count):
                ...
                # 查询项目的pods信息
                r = multi_cluster_steps.step_get_pods_of_project(cluster_name=cluster_name, project_name=project_name)
                # 获取pod的数量
    ...
    def test_precise_query_existent_pod_of_project(self):
        ...
        for i in range(0, cluster_count):
            ...
            # 查询项目的所有的pod信息
            re = multi_cluster_steps.step_get_pods_of_project(cluster_name=cluster_name, project_name=project_name)
            # 获取pod的数量
            ...
            # 使用pod的名称，精确查询存在的pod
            r = multi_cluster_steps.step_get_pods_of_project(cluster_name, project_name, 'name=' + pod_names[0])
            # 获取查询结果中pod名称
    ...
    def test_fuzzy_query_existent_pod_of_project(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, pod_count):
                ...
                pod_names.append(name)
            # 使用pod的名称，模糊查询存在的pod
            r = multi_cluster_steps.step_get_pods_of_project(cluster_name, project_name, 'name=' + pod_names[0][2:])
            # 获取查询结果中pod名称
            pod_name_actual = r.json()['items'][0]['metadata']['name']
            # 验证查询结果正确
            assert pod_name_actual == pod_names[0]
    ...
    def test_fuzzy_query_existent_pod_of_project(self):
        for cluster_name in self.cluster_names:
            ...
            r = multi_cluster_steps.step_get_pods_of_project(cluster_name, project_name, 'name=' + pod_name)
            # 获取查询结果中的pod数量
            pod_count = r.json()['totalItems']
            # 验证查询结果正确
            assert pod_count == 0
    ...
    def test_create_user_system(self):
        for cluster_name in self.cluster_names:
            ...
            # 创建用户项目
            multi_cluster_steps.step_create_user_project(cluster_name, project_name, alias_name, description)
            # 查询创建的项目，并获取其运行状态
            r = multi_cluster_steps.step_get_user_system(cluster_name, project_name)
            state = r.json()['status']['phase']
            # 验证项目的状态为active
            assert state == 'Active'
            # 删除创建的项目
            multi_cluster_steps.step_delete_user_system(cluster_name, project_name)
    ...
    def test_delete_user_system(self):
        for cluster_name in self.cluster_names:
            ...
            multi_cluster_steps.step_create_user_project(cluster_name, project_name, alias_name, description)
            # 删除用户项目
            re = multi_cluster_steps.step_delete_user_system(cluster_name, project_name)
            # 获取删除项目的状态
    ...
    def test_check_daemonsets_of_cluster(self):
        for cluster_name in self.cluster_names:
            # 查询集群所有的daemonSets
            re = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, 'daemonsets')
            # 获取集群daemonSets的数量
            count = re.json()['totalItems']
    ...
    def test_check_app_workload_revision_records(self, type, title):
        ...
        for i in range(0, cluster_count):
            ...
            cluster_name = response.json()['items'][i]['metadata']['name']
            # 查询集群所有的daemonSets
            res = multi_cluster_steps.step_get_resource_of_cluster(cluster_name=cluster_name, resource_type=type)
            # 获取集群daemonSets的数量
    ...
    def test_fuzzy_query_app_workload_by_name(self, type, title):
        for cluster_name in self.cluster_names:
            ...
            # 获取第一个资源的名称
            name = re.json()['items'][0]['metadata']['name']
            r = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, type, 'name=' + name[2:])
            # 获取查询结果的名称
            name_actual = r.json()['items'][0]['metadata']['name']
            # 验证查询结果正确
            assert name == name_actual
    ...
    def test_query_app_workload_by_status(self, type, title):
        for cluster_name in self.cluster_names:
            # 查询状态为running的资源
            r = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, type, 'status=running')
            # 获取资源的数量
    ...
    def test_get_pods_of_cluster(self):
        for cluster_name in self.cluster_names:
            # 获取集群的容器组的数量
            res = multi_cluster_steps.step_get_pods_of_cluster(cluster_name)
            count = res.json()['totalItems']
            # 从项目管理处获取所有项目的名称
            project_name = []
            re = multi_cluster_steps.step_get_project_of_cluster(cluster_name)
            project_count = re.json()['totalItems']
            for j in range(0, project_count):
                name = re.json()['items'][j]['metadata']['name']
            ...
            for project in project_name:
                r = multi_cluster_steps.step_get_pods_of_project(cluster_name, project)
                pod_count = r.json()['totalItems']
    ...
    def test_precise_query_resource_by_name(self, story, type, title):
        ...
        for cluster_name in self.cluster_names:
            # 获取集群中存在的任一资源的名称
            re = multi_cluster_steps.step_get_resource_of_cluster(cluster_name=cluster_name, resource_type=type)
            count = re.json()['totalItems']
    ...
    def test_fuzzy_query_resource_by_name(self, story, type, title):
        ...
        for cluster_name in self.cluster_names:
            # 查看集群中的某一种资源
            re = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, resource_type=type)
            # 获取集群中某一种资源的数量
            count = re.json()['totalItems']
            # 获取集群中存在的任一资源的名称
            name = re.json()['items'][random.randint(0, count - 1)]['metadata']['name']
            fuzzy_name = name[1:]
            # 按名称精确查询存在的资源
            r = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, type, 'name=' + fuzzy_name)
            name_actual = r.json()['items'][0]['metadata']['name']
            # 验证查询结果正确
            assert name_actual == name
    ...
    def test_get_crd_detail(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, count):
                name = re.json()['items'][j]['metadata']['name']
                # 查询CRD的详情信息
                r = multi_cluster_steps.step_get_crd_detail(cluster_name, name)
                # 获取查询结果中的kind
                kind = r.json()['kind']
                # 验证查询结果正确
    ...
    def test_query_pvc_by_status(self):
        for cluster_name in self.cluster_names:
            # 查询状态为bound的存储卷
            r = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, 'persistentvolumeclaims', 'status=bound')
            # 获取存储卷数量
            count = r.json()['totalItems']
            # 获取并验证所有存储卷的状态为Bound
            for j in range(0, count):
    ...
    def test_get_pvc_detail(self):
        for cluster_name in self.cluster_names:
            # 查询集群的存储卷
            re = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, 'persistentvolumeclaims')
            # 获取存储卷的数量
            count = re.json()['totalItems']
            ...
            for j in range(0, count):
                ...
                # 查询存储卷的详情信息
                r = multi_cluster_steps.step_get_pvc_detail(cluster_name, namespace, name)
                # 获取查询结果的kind
                kind = r.json()['kind']
                # 验证查询结果正确
    ...
    def test_get_pvc_pods(self):
        for cluster_name in self.cluster_names:
            ...
            for j in range(0, count):
                ...
                namespace = re.json()['items'][j]['metadata']['namespace']
                # 查询存储卷的pod信息
                r = multi_cluster_steps.step_get_pods_of_project(cluster_name, namespace, 'pvcName=' + name)
                # 获取pod的数量
    ...
    def test_get_storage_class_detail(self):
        for cluster_name in self.cluster_names:
            # 查询集群存在的存储类型
            re = multi_cluster_steps.step_get_resource_of_cluster(cluster_name, 'storageclasses')
            # 获取存储类型的数量
            count = re.json()['totalItems']
            # 获取所有的存储类型的名称
            for j in range(0, count):
                name = re.json()['items'][j]['metadata']['name']
                # 查询每个存储类型的详情信息
                r = multi_cluster_steps.step_get_storage_class_detail(cluster_name, name)
                # 获取详情信息中的kind
                kind = r.json()['kind']
                # 验证查询结果正确
    ...
    def test_set_default_storage_class(self):
        for cluster_name in self.cluster_names:
            ...
            name = re.json()['items'][random.randint(0, count - 1)]['metadata']['name']
            # 将任一存储类型设置为非默认存储类型
            r = multi_cluster_steps.step_set_default_storage_class(cluster_name, name, 'false')
            ...
            # 将任一存储类型设置为默认存储类型
            r = multi_cluster_steps.step_set_default_storage_class(cluster_name, name, 'true')
            # 获取请求结果中的storageclass.kubernetes.io/is-default-class
            result = r.json()['metadata']['annotations']['storageclass.kubernetes.io/is-default-class']
            # 验证结果为true
            assert result == 'true'
    ...
    def test_get_component_health(self):
        for cluster_name in self.cluster_names:
            # 获取组件的健康状况
            r = multi_cluster_steps.step_get_component_health(cluster_name)
            # 获取组件的数量
    ...
    def test_get_node_usage_rank(self, sort, title):
        for cluster_name in self.cluster_names:
            ...
            r = multi_cluster_steps.step_get_node_usage_rank(cluster_name, sort)
            # 获取结果中的数据类型
            for j in range(0, 15):
                try:
                    result_type = r.json()['results'][j]['data']['resultType']
