...
class TestMetering(object):
    ...
    def test_get_cluster_consumption_by_yesterday(self, step, title):
        ...
        for i in range(0, cluster_count):
            ...
            # 获取截止到昨天的最近7天的消费历史
            re = step_get_cluster_consumption_history(type='cluster', start_time=before_timestamp,
    ...
    def test_get_node_consumption(self, metric_name, title):
        ...
        for i in range(0, cluster_count):
            ...
            cluster_name = response.json()['items'][i]['metadata']['name']
            # 查询集群的节点信息
            re = step_get_node_info(cluster_name)
            # 获取集群节点的数量
    ...
    def test_get_node_consumption_by_yesterday(self, step, title):
        ...
        for i in range(0, cluster_count):
            ...
            for j in range(0, count):
                name = re.json()['items'][j]['metadata']['name']
                # 获取截止到昨天的最近7天的消费历史
                r = step_get_node_consumption_history(cluster_name=cluster_name, start_time=before_timestamp,
        ...
        for i in range(0, cluster_count):
            ...
            for j in range(0, count):
                try:
                    name = re.json()['items'][j]['metadata']['name']
                    # 查询pod的消费信息
                    r = step_get_pod_consumption(cluster_name=cluster_name, node_name=name, metric=metric_name)
                    # 获取metric_name
    ...
    def test_get_pod_consumption_by_yesterday(self):
        ...
        for i in range(0, cluster_count):
            ...
            cluster_name = response.json()['items'][i]['metadata']['name']
            # 查询集群所有的节点
            res = step_get_node_info(cluster_name)
            # 获取集群的节点数量
            ...
            for m in range(0, node_count):
                ...
                # 查询每个节点的pod信息
                re = step_get_pod_info(cluster_name, node_name)
                # 获取每个节点的pod数量
                ...
                for j in range(0, pod_count):
                    ...
                    # 查询pod截止到昨天最近7天的消费情况
                    r = step_get_pod_consumption_history(cluster_name, node_name, before_timestamp,
        ...
        for i in range(0, ws_count):
            ...
            if ws_name != 'system-workspace':
                ...
                for k in range(0, cluster_count):
                    ...
                    # 查询企业空间的历史消费信息
                    r = step_get_workspace_consumption_history(cluster_name, ws_name, before_timestamp,
                                                               now_timestamp, step)
                    # 查询每个指标的消费历史数据
                    for j in range(0, 5):
                        try:
        ...
        for i in range(0, ws_count):
            ...
            if ws_name != 'system-workspace':
                ...
                for k in range(0, cluster_count):
                    cluster_name = response.json()['items'][i]['spec']['placement']['clusters'][k]['name']
                    # 查询每个企业空间的项目信息
                    re = step_get_project_info(cluster_name, ws_name)
                    # 获取项目的数量
            ...
            if ws_name != 'system-workspace':
                ...
                for k in range(0, cluster_count):
                    ...
                    for j in range(0, project_count):
                        ...
                        r = step_get_project_consumption_history(cluster_name, project_name,
            ...
            if ws_name != 'system-workspace':
                ...
                for k in range(0, cluster_count):
                    ...
                    for j in range(0, project_count):
                        project_name = re.json()['items'][j]['metadata']['name']
                        # 查询最近1h消费的资源信息
                        r = step_get_hierarchy_consumption(cluster_name, ws_name, project_name)
                        # 验证资源查询成功
                        ...
                        for k in hierarchy_list:
                            if r.json()[k] is not None:
                                ...
                                for key in hierarchy.keys():
                                    # 查询资源截止到昨天的最近7天消费历史
                                    rep = step_get_hierarchy_consumption_history(cluster_name, project_name,
