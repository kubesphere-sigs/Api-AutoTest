import requests
import pytest
import json
import allure
import sys
import random
import time
from common.getData import DoexcleByPandas

sys.path.append('../')  #将项目路径加到搜索路径中，使得自定义模块可以引用

from config import config
from common.getHeader import get_header, get_header_for_patch
from common import commonFunction

@allure.step('创建应用模板')
def step_create_app_template(self):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + self.ws_name + '/apps'
    data = {"version_type": "helm",
            "name": "testapp2",
            "version_package": ""
            }
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.status_code == 200

@allure.step('部署应用')
def step_deploy_template(ws_name, project_name):
    """
    :param ws_name:  工作空间名称
    :param project_name:  项目名称
    :return: 部署应用时输入的名称
    """
    #获取应用模板的app_id和version_id
    name = 'testapp' + str(int(random.random()*100))  #拼接一个随机数名称
    app_id, version_id = commonFunction.get_app_template(ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/clusters/default/namespaces/' + project_name + '/applications'
    data = {"app_id": app_id,
            "name": name,
            "version_id": version_id, "runtime_id": "",
            "conf": "affinity: {}\napiserver: 'https://kubernetes.default.svc:443'\nemqxAddressType: ip\nemqxConfig:\n  EMQX_MANAGEMENT__LISTENER__HTTP: '8080'\nemqxLicneseSecretName: null\nimage: 'emqx/emqx:latest'\nimagePullPolicy: IfNotPresent\ningress:\n  annotations: {}\n  enabled: false\n  hosts:\n    - chart-example.local\n  path: /\n  tls: []\nnamespace: default\nnodeSelector: {}\npersistence:\n  accessMode: ReadWriteOnce\n  enabled: false\n  size: 20Mi\nreplicas: 3\nresources: {}\nservice:\n  type: ClusterIP\ntolerations: []\n"
            "Name: " + name +"\nDescription: ''\nWorkspace: " + ws_name + "\n"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'
    return name  #返回部署时应用的名称

@allure.step('查看应用状态')
def step_get_app_status(ws_name, project_name, app_name):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/clusters/default/namespaces/' + project_name + '/applications' \
                       '?conditions=status%3Dactive%7Cstopped%7Cpending%7Csuspended%2Ckeyword%3D' + app_name + '&paging=limit%3D10%2Cpage%3D1&orderBy=status_time&reverse=true'

    r = requests.get(url=url, headers=get_header())
    # print(r.json()['items'][0]['cluster']['status'])
    return r.json()['items'][0]['cluster']['status']

@allure.step('查看应用的cluster_id')
def step_get_app_cluster_id(ws_name, project_name, app_name):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/clusters/default/namespaces/' + project_name + '/applications' \
                       '?conditions=status%3Dactive%7Cstopped%7Cpending%7Csuspended%2Ckeyword%3D' + app_name + '&paging=limit%3D10%2Cpage%3D1&orderBy=status_time&reverse=true'

    r = requests.get(url=url, headers=get_header())
    print(r.json()['items'][0]['cluster']['status'])
    return r.json()['items'][0]['cluster']['cluster_id']

@allure.step('删除部署的应用')
def step_delete_app(ws_name, project_name, cluster_id):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/clusters/default/namespaces/' + project_name + '/applications/' + cluster_id

    r = requests.delete(url=url, headers=get_header())
    assert r.json()['message'] == 'success'

@allure.step('应用模板提交审核')
def step_app_template_submit(self):
    # 获取应用模板的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')

    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/'
    data = {"name": "1.1",
            "description": "testapp2 helm chart description"}
    requests.patch(url, headers=get_header_for_patch(), data=json.dumps(data))

    url2 = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data2 = {"action": "submit"}
    r = requests.post(url2, headers=get_header(), data=json.dumps(data2))
    assert r.json()['message'] == 'success'

@allure.step('应用模板撤销审核')
def step_app_template_submit_cancle(self):
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data = {"action": "cancel"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('应用审核--通过')
def step_app_pass(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')

    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data = {"action": "pass"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('应用审核--不通过')
def step_app_reject(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')

    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data = {"action": "reject", "message": "test-reject"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('发布应用商店')
def step_release(self):
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data = {"action": "release"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('下架应用')
def step_suspend_app(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/action'
    data = {"action": "suspend"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('重新上架')
def step_app_recover(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/action'
    data = {"action": "recover"}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.json()['message'] == 'success'

@allure.step('查看应用审核记录')
def step_audit_records(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version_id + '/audits?orderBy=status_time&paging=limit%3D200%2Cpage%3D1&conditions=status%3D&reverse=true'
    r = requests.get(url, get_header())
    assert r.json()['total_count'] >= 1

@allure.step('添加版本')
def step_add_version(self):
    # 获取应用的app_id和version_id
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + self.ws_name + '/apps/' + app_id + '/versions'
    data = {"type": "helm",
            "package": ""}
    r = requests.post(url, headers=get_header(), data=json.dumps(data))
    assert r.status_code == 200

@allure.step('删除版本')
def step_delete_version(self):
    app_id, version_id = commonFunction.get_app_template(self.ws_name, 'testapp2')
    versions = commonFunction.get_app_versions(self.ws_name, app_id)
    for version in versions:
        url = config.url + '/kapis/openpitrix.io/v1/apps/' + app_id + '/versions/' + version + '/'
        data = {}
        r = requests.delete(url, headers=get_header(), data=json.dumps(data))  # 删除应用版本
        assert r.json()['message'] == 'success'

@allure.step('删除应用模板')
def step_delete_app_template(self):
    app_id = commonFunction.get_app_id_noversion(self.ws_name, 'testapp2')
    data = {}
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + self.ws_name + '/apps/' + app_id + '/'
    r = requests.delete(url, headers=get_header(), data=json.dumps(data))  #删除应用模板
    r.json()['message'] == 'success'

@allure.step('查找指定的应用')
def step_get_app(ws_name, project_name, app_name):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/clusters/default/namespaces/' + project_name + '/applications' \
                        '?conditions=status%3Dactive%7Cstopped%7Cpending%7Csuspended%2Ckeyword%3D' + app_name + '&paging=limit%3D10%2Cpage%3D1&orderBy=status_time&reverse=true'
    r = requests.get(url=url, headers=get_header())
    print(r.json()['total_count'])
    return r.json()['total_count']

@allure.step('添加仓库')
def step_add_app_repository(ws_name, rpo_name, rpo_url):
    """
    :param ws_name: 企业空间
    :param rpo_name: 应用仓库名称
    :param rpo_url: 应用仓库地址
    :return: 应用仓库的repo_id
    """
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/repos'
    data = {"name": rpo_name,
            "repoType": "Helm",
            "type": "https",
            "visibility": "public",
            "credential": "{}",
            "providers": ["kubernetes"],
            "url": rpo_url,
            "app_default_status": "active"
            }
    r = requests.post(url=url, headers=get_header(), data=json.dumps(data))
    return r.json()['repo_id']

@allure.step('查询应用仓库列表指定仓库')
def step_get_app_reposity(ws_name, rpo_name, except_result):
    """
    :param except_result: 预期的查询结果数量
    :param ws_name: 企业空间
    :param rpo_name: 应用仓库名称
    :return: 应用仓库的repo_id
    """
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/repos' \
                       '?orderBy=create_time&paging=limit%3D10%2Cpage%3D1&conditions=status%3Dactive%2Ckeyword%3D' + rpo_name + '&reverse=true'
    r = requests.get(url=url, headers=get_header())
    assert r.json()['total_count'] == except_result  #验证查询结果
    #如果查询结果不为空，才返回对应的应用仓库的repo_id
    if except_result == 1:
        return r.json()['items'][0]['repo_id']

@allure.step('删除应用仓库')
def step_delete_app_repo(ws_name, repo_id):
    url = config.url + '/kapis/openpitrix.io/v1/workspaces/' + ws_name + '/repos/' + repo_id + '/'
    r = requests.delete(url=url, headers=get_header())
    #验证删除请求成功
    return r.text

@allure.feature('应用管理')
class TestAppTemplate(object):
    ws_name = 'test-ws1'   #在读取excle中的测试用例时，使用到了ws_name
    project_name = 'test-project1'
    repo_name = 'test-rpo'  #应用仓库的名称
    repo_url = 'https://helm-chart-repo.pek3a.qingstor.com/kubernetes-charts/'  #应用仓库的地址

    # 所有用例执行之前执行该方法
    def setup_class(self):
        commonFunction.create_workspace(self.ws_name)  #创建一个企业空间
        commonFunction.create_project(ws_name=self.ws_name, project_name=self.project_name)  #创建一个项目

    # 所有用例执行完之后执行该方法
    def teardown_class(self):
        commonFunction.delete_project(self.ws_name, self.project_name)  #删除创建的项目
        commonFunction.delete_workspace(self.ws_name)  # 删除创建的企业空间

    parametrize = DoexcleByPandas().get_data_for_pytest(filename='../data/data.xlsx', sheet_name='appmanage')

    @allure.title('{title}')  # 设置用例标题
    @allure.severity(allure.severity_level.CRITICAL)  # 设置用例优先级
    # 将用例信息以参数化的方式传入测试方法
    @pytest.mark.parametrize('id,url,data,story,title,method,severity,condition,except_result', parametrize)
    def test_app_repo(self, id, url, data, story, title, method, severity, condition, except_result):

        """
        :param id: 用例编号
        :param url: 用例请求的URL地址
        :param data: 用例使用的请求数据
        :param story: 用例模块
        :param title: 用例标题
        :param method: 用例的请求方式
        :param severity: 用例优先级
        :param condition: 用例的校验条件
        :param condition: 用例的校验条件
        :param except_result: 用例的预期结果
        """

        allure.dynamic.story(story)  # 动态生成模块
        allure.dynamic.severity(severity)  # 动态生成用例等级

        # test开头的测试函数
        url = config.url + url
        if method == 'get':
            # 测试get方法
            r = requests.get(url, headers=get_header())

        elif method == 'post':
            # 测试post方法
            data = eval(data)
            r = requests.post(url, headers=get_header(), data=json.dumps(data))

        elif method == 'patch':
            # 测试patch方法
            data = eval(data)
            print(data)
            r = requests.patch(url, headers=get_header_for_patch(), data=json.dumps(data))

        elif method == 'delete':
            # 测试delete方法
            r = requests.delete(url, headers=get_header())

        # 将校验条件和预期结果参数化
        if condition != 'nan':
            condition_new = eval(condition)  # 将字符串转化为表达式
            if isinstance(condition_new, str):
                # 判断表达式的结果是否为字符串，如果为字符串格式，则去掉其首尾的空格
                assert condition_new.strip() == except_result
            else:
                assert condition_new == except_result

        # 将用例中的内容打印在报告中
        print(
            '用例编号: ' + str(id) + '\n'
            '用例请求的URL地址: ' + str(url) + '\n'
            '用例使用的请求数据: ' + str(data) + '\n'
            '用例模块: ' + story + '\n'
            '用例标题: ' + title + '\n'
            '用例的请求方式: ' + method + '\n'
            '用例优先级: ' + severity + '\n'
            '用例的校验条件: ' + str(condition) + '\n'
            '用例的实际结果: ' + str(condition_new) + '\n'
            '用例的预期结果: ' + str(except_result)
        )


    @allure.story('应用管理-应用仓库')
    @allure.title('添加仓库时，输入正确的条件')
    @allure.severity(allure.severity_level.BLOCKER)
    def test_add_app_repository(self):
        #添加仓库
        step_add_app_repository(self.ws_name, self.repo_name, self.repo_url)
        #查询列表，验证仓库添加成功
        step_get_app_reposity(self.ws_name, self.repo_name, 1)

    @allure.story('应用管理-应用仓库')
    @allure.title('删除存在的应用仓库')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_delete_app_repository(self):
        #该用例依赖于用例'添加仓库时，输入正确的条件'
        #验证删除的应用仓库存在并获取repo_id
        repo_id = step_get_app_reposity(self.ws_name, self.repo_name, 1)
        #删除应用
        message = step_delete_app_repo(self.ws_name, repo_id)
        #验证删除结果
        assert 'success' in message
        time.sleep(5)
        #在应用仓库列表查询被删除的应用仓库，结果为null
        step_get_app_reposity(self.ws_name, self.repo_name, 0)

    @allure.story('应用管理-应用仓库')
    @allure.title('删除不存在的应用仓库')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_delete_app_repository_no(self):
        # 验证删除的应用仓库不存在
        repo_id = 'qwe'
        # 删除应用仓库
        message = step_delete_app_repo(self.ws_name, repo_id)
        #验证删除结果
        assert message == 'rpc error: code = NotFound desc = resource [[' + repo_id + ']] not found\n'

    @allure.story('应用管理-应用模板')
    @allure.title('发布应用模板到商店，然后将应用下架，再重新上架，最后下架应用')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_suspend_app_template(self):
        step_create_app_template(self)  #创建应用模板
        step_app_template_submit(self)  #应用模板提交审核
        step_app_pass(self)  #应用审核通过
        step_release(self)  #发布模板到应用商店
        step_suspend_app(self)   #下架应用
        step_app_recover(self)  #重新上架应用
        step_suspend_app(self)  # 下架应用

        step_delete_version(self)  # 删除应用版本
        step_delete_app_template(self)  # 删除应用模板

    @allure.story('应用管理-应用模板')
    @allure.title('应用审核不通过,然后重新提交审核，最后审核通过')
    @allure.severity(allure.severity_level.CRITICAL)
    def test_app_check_reject(self):
        step_create_app_template(self)  # 创建应用模板
        step_app_template_submit(self)  # 应用模板提交审核
        step_app_reject(self)  #应用审核不通过
        step_app_template_submit(self)  # 应用模板提交审核
        step_app_pass(self)  # 应用审核通过
        step_audit_records(self)  #查看应用审核记录

        step_delete_version(self)  # 删除应用版本
        step_delete_app_template(self)  # 删除应用模板

    @allure.story('应用管理-应用模板')
    @allure.title('创建应用模板后添加版本')
    def test_add_version(self):
        step_create_app_template(self)  # 创建应用模板
        step_add_version(self)  #添加应用版本

        step_delete_version(self)  # 删除应用版本
        step_delete_app_template(self)  # 删除应用模板

    @allure.story('应用管理-应用模板')
    @allure.title('测试部署创建的应用模板')
    def test_deployment_app_template(self):
        step_create_app_template(self)  # 创建应用模板
        app_name = step_deploy_template(self.ws_name, self.project_name)  #测试部署应用模板
        print(app_name)
        # 在项目的应用列表中验证部署的应用运行正常,最长等待时间600s
        i = 0
        while i < 600:
            status = step_get_app_status(self.ws_name, self.project_name, app_name)
            if status == 'active':
                print('应用部署耗时:' + str(i) + '秒')
                break
            time.sleep(10)
            i = i + 10
        assert status == 'active'
        #获取部署的应用的cluster_id
        cluster_id = step_get_app_cluster_id(self.ws_name, self.project_name, app_name)
        print(cluster_id)
        #删除创建的应用
        step_delete_app(self.ws_name, self.project_name, cluster_id)
        #在应用列表中查询创建的应用，验证删除成功
        time.sleep(5)  #等待删除时间
        count = step_get_app(self.ws_name, self.project_name, app_name)
        assert count == 0

        step_delete_version(self)  # 删除应用版本
        step_delete_app_template(self)  # 删除应用模板






if __name__ == "__main__":
    pytest.main(['-s', 'testAppManage.py'])  # -s参数是为了显示用例的打印信息。 -q参数只显示结果，不显示过程








