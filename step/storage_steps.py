...
def search_volume_by_name(volume_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/persistentvolumeclaims?name='+volume_name+'&sortBy=createTime&limit=10'
    response = requests.get(url=url, headers=get_header())
    return response


@allure.step('创建存储类')
def create_sc(sc_name, expansion):
    url = env_url + '/apis/storage.k8s.io/v1/storageclasses'
...
@allure.step('创建授权规则')
def create_sc_accessor(sc_name):
    url = env_url + '/apis/storage.kubesphere.io/v1alpha1/accessors'
...
def open_sc_accessor(sc_name, enable):
    url = env_url + '/apis/storage.kubesphere.io/v1alpha1/accessors/' + sc_name + '-accessor'
...
@allure.step('删除授权规则')
def delete_sc_accessor(sc_name):
    url = env_url + '/apis/storage.kubesphere.io/v1alpha1/accessors/' + sc_name + '-accessor'
...
@allure.step('删除存储类')
def delete_sc(sc_name):
    url = env_url + '/apis/storage.k8s.io/v1/storageclasses/' + sc_name
    response = requests.delete(url, headers=get_header())
    return response


@allure.step('按名称查询存储类')
def search_sc_by_name(sc_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/storageclasses?name=' + sc_name + '&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response


@allure.step('查询存储类信息')
def get_sc_info(sc_name):
    url = env_url + '/apis/storage.k8s.io/v1/storageclasses/' + sc_name
    response = requests.get(url, headers=get_header())
    return response
...
def search_volume_by_sc(sc_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/persistentvolumeclaims?storageClassName=' + sc_name + '&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response


@allure.step('在存储类详情查询存储卷')
def search_volume(sc_name, volume_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/persistentvolumeclaims?storageClassName=' + sc_name + \
...
@allure.step('设置存储类授权规则')
def set_sc_ws_accessor(sc_name, ws_accessor, scn):
...
@allure.step('设置卷操作')
def set_volume_operations(sc_name, expansion, allow_clone, allow_snapshot):
    url = env_url + '/apis/storage.k8s.io/v1/storageclasses/' + sc_name
...
@allure.step('设置默认存储类')
def set_default_sc(sc_name, expansion):
    url = env_url + '/apis/storage.k8s.io/v1/storageclasses/' + sc_name
...
@allure.step('创建存储卷')
def step_create_volume(project_name, sc_name, volume_name):
...
@allure.step('创建卷快照类')
def create_vsc(vsc_name, driver, policy):
    url = env_url + '/apis/snapshot.storage.k8s.io/v1beta1/volumesnapshotclasses'
...
@allure.step('编辑卷快照类')
def set_vsc(vsc_name, driver, policy, creationtime, time, generation, version, uid, alias_name, des):
    url = env_url + '/apis/snapshot.storage.k8s.io/v1beta1/volumesnapshotclasses/' + vsc_name
    data = {"apiVersion": "snapshot.storage.k8s.io/v1beta1",
            "deletionPolicy": policy,
            "driver": driver,
            "kind": "VolumeSnapshotClass",
...
def search_vsc_by_name(vsc_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/volumesnapshotclasses?name=' + vsc_name + '&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response


@allure.step('获取卷快照类已有快照')
def get_vs_for_vsc(vsc_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/volumesnapshots?volumeSnapshotClassName=' + vsc_name + '&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response


@allure.step('查询卷快照类已有快照')
def search_vs_for_vsc(vsc_name, vs_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/volumesnapshots?volumeSnapshotClassName=' + vsc_name + '&name=' + vs_name+'&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response
...
@allure.step('创建快照')
def create_volume_snapshots(project, vs_name, vsc_name, volume_name):
    url = env_url + '/apis/snapshot.storage.k8s.io/v1beta1/namespaces/' + project + '/volumesnapshots'
    data = {"apiVersion": "snapshot.storage.k8s.io/v1beta1",
            "kind": "VolumeSnapshot",
...
def search_vs_by_name(vs_name):
    url = env_url + '/kapis/resources.kubesphere.io/v1alpha3/volumesnapshots?name=' + vs_name + '&sortBy=createTime&limit=10'
    response = requests.get(url, headers=get_header())
    return response


@allure.step('删除卷快照')
def delete_vs(project_name, vs_name):
    url = env_url + '/apis/snapshot.storage.k8s.io/v1beta1/namespaces/'+project_name+'/volumesnapshots/' + vs_name
    response = requests.delete(url, headers=get_header())
    return response


@allure.step('通过卷快照创建卷')
def create_volumne_by_vs(project_name, volume_name, sc_name):
    url = env_url + '/api/v1/namespaces/'+project_name+'/persistentvolumeclaims'
    data = {"apiVersion": "v1",
            "kind": "PersistentVolumeClaim",
            "metadata": {"namespace": project_name,
